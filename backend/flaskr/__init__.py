...
def create_app(test_config=None):
    ...
    def after_request(response): #must return the response object
        ...
        response.headers.add('Access-Control-Allow-Headers','POST,PATCH,GET,DELETE,OPTIONS') #allowed methods from cross domains
    ...
    def paginate(request,results):
        ...
        objects = [obj.format() for obj in results] #format the object into nice string representation
    ...
    def get_all_questions():
        questions = Question.query.order_by(Question.id).all()#fetch and order questions in order of creation
        ques_count =len(questions)
        question = paginate(request,questions) #all formated questions
        if len(question)==0:
